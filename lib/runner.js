...
function Runner(options/*, settings*/) {
  ...
  var parseModule = function(module) {
    ...
    if (Number(module.error) === 0) {
      ...
      var message = module.error['format-string'];
      if (module.error.stack !== undefined) {
        message += ' - file [' + module.error.stack.frame[0].uri + '] - line [' + module.error.stack.frame[0].line + '] - column [' + module.error.stack.frame[0].column + ']';
      }
  ...
  this.getRequestOptions = function(module) {
    ...
    options.url = this.options.url + (endsWith(this.options.url, '/')?'':'/');
    options.qs = {'dir': this.options.testDir, 'modules': module, 'format': 'json'};
    return options
  };
